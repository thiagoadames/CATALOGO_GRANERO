...
def clasificar_producto(nombre_producto, categorias_disponibles):
    # Ahora recibe la lista de categorías reales de la tienda (dicts con id_categoria y nombre_categoria)
    # y devuelve el id_categoria correspondiente, ya que productos.categoria es INT (FK)
...
def registrar_tienda():
    if request.method == 'POST':
        ...
        try:
            ...
            # --- CORRECCIÓN REALIZADA: Eliminada la columna 'city' para coincidir con la BD ---
            ...
            # Ajuste a 11 valores (eliminando el parámetro extra 'city')
...
def panel_administrador(id_tienda):
    ...
    try:
        ...
        # Ejecutamos búsqueda limpia de productos vinculados, con JOIN para traer el nombre real de la categoría
        cursor.execute("""
...
def agregar_producto(id_tienda):
    ...
    try:
        ...
        # Obtenemos las categorías propias de esta tienda (para el select y para la IA)
        ...
        if request.method == 'POST':
            ...
            # Usamos IA si no envían categoría; clasificar_producto ahora devuelve un id_categoria
        ...
        cursor.close()
        conexion.close()
        return render_template('agregar_producto.html', id_tienda=id_tienda, lista_categorias=categorias_tienda)
...
def editar_producto(id_tienda, id_producto):
    ...
    try:
        ...
        # Obtenemos las categorías propias de esta tienda para el select
        cursor.execute("SELECT id_categoria, nombre_categoria FROM categorias WHERE id_tienda = %s", (id_tienda,))
        categorias_tienda = cursor.fetchall()
...
def ver_catalogo(id_tienda):
    try:
        ...
        # Traemos las categorías de la tienda para el menú de filtros del catálogo público
        cursor.execute("SELECT id_categoria, nombre_categoria FROM categorias WHERE id_tienda = %s", (id_tienda,))
        categorias_tienda = cursor.fetchall()
...
# =========================================================================
# RUTA: GESTIONAR CATEGORÍAS PROPIAS DE LA TIENDA (LISTAR + AGREGAR)
# =========================================================================
...
def gestionar_categorias(id_tienda):
    ...
    try:
        ...
        return render_template('categorias.html', id_tienda=id_tienda, lista_categorias=lista_categorias)

    except mysql.connector.Error as err:
        return f"<h1>Error al gestionar categorías: {err}</h1>"


# =========================================================================
# RUTA: EDITAR CATEGORÍA
# =========================================================================
@app.route('/tienda/<int:id_tienda>/categorias/editar/<int:id_categoria>', methods=['POST'])
def editar_categoria(id_tienda, id_categoria):
    if not session.get('admin_logeado'):
        return redirect(url_for('login'))
    ...
    try:
        ...
        cursor.execute("UPDATE categorias SET nombre_categoria = %s WHERE id_categoria = %s AND id_tienda = %s", (nuevo_nombre, id_categoria, id_tienda))
        conexion.commit()
        ...
        return f"<h1>Error al editar categoría: {err}</h1>"


# =========================================================================
# RUTA: ELIMINAR CATEGORÍA
# =========================================================================
@app.route('/tienda/<int:id_tienda>/categorias/eliminar/<int:id_categoria>')
